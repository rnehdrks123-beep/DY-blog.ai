...
// ==========================================
// [ API KEY 설정 ]
// - AI Studio의 Settings > Secrets에 GEMINI_API_KEY가 등록되어 있으면 자동으로 서버에서 해당 키를 사용합니다.
...
export default function App() {
  ...
  const handleGenerateReply = async () => {
    ...
    try {
      const response = await fetch("/api/generate-reply", {
        ...
        body: JSON.stringify({
          review: reviewInput,
          userApiKey: API_KEY, // 빈 문자열인 경우 서버의 환경 변수를 사용함
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || "답글 생성 도중 오류가 발생했습니다.");
      }

      setReplyOutput(data.result);
      triggerToast("✨ 감동을 담은 맞춤형 답글이 생성되었습니다!");
    } catch (err: any) {
      setReplyError(err.message || "서버와 통신하는 도중 에러가 발생했습니다.");
    } finally {
  ...
  const handleGenerateNews = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "소식 기획 도중 오류가 발생했습니다.");
      }

      setNewsOutput(data.result);
      triggerToast("📝 플레이스에 등록할 매력적인 소식이 완성되었습니다!");
    } catch (err: any) {
      setNewsError(err.message || "서버와 통신하는 도중 에러가 발생했습니다.");
    } finally {
